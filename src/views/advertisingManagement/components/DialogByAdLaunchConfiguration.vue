<template>
  <el-dialog :model-value="computedVisible" align-center destroy-on-close :before-close="beforeClose" @open="init()">
    <template #header>
      <h2>{{ $props.doType == 1 ? "热" : "冷" }}启配置</h2>
    </template>

    <el-form ref="dialogFormRef" :model="dialogForm" status-icon label-position="top" :rules="formRules">
      <el-form-item label="间隔时长" prop="time" v-if="$props.doType == 1">
        <el-input v-model="dialogForm.time" />
      </el-form-item>
      <el-form-item label="状态" prop="status">
        <el-switch v-model="dialogForm.status" size="large" inline-prompt active-text="开启" inactive-text="关闭" />
      </el-form-item>
    </el-form>
